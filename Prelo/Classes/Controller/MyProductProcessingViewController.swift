...
class MyProductProcessingViewController : BaseViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.refreshControl.addTarget(self, action: #selector(MyProductProcessingViewController.refresh(_:)), forControlEvents: UIControlEvents.ValueChanged)
    ...
    func getUserProducts() {
        // API Migrasi
        request(APITransaction.Sells(status: "process", current: "\(nextIdx)", limit: "\(nextIdx + ItemPerLoad)")).responseJSON {resp in
