...
class MyProductProcessingViewController : BaseViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.refreshControl.addTarget(self, action: #selector(MyProductProcessingViewController.refresh(_:)), for: UIControlEvents.valueChanged)
    ...
    func getUserProducts() {
        // API Migrasi
        let _ = request(APITransaction.sells(status: "process", current: "\(nextIdx)", limit: "\(nextIdx + ItemPerLoad)")).responseJSON {resp in
